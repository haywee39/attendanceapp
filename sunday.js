        ...
        function isSunday(dateString) {
            ...
            return date.getDay() === 0; // 0 is Sunday, 1 is Monday, 2 is Tuesday
        }
        ...
        eventDateInput.addEventListener('change', function() {
            if (this.value) {
                if (!isSunday(this.value)) {
                    dateError.textContent = 'Day is NOT a Sunday!.';
                    this.setCustomValidity('ENTER a SUNDAY date!.'); // Mark as invalid
                } else {
        ...
        dataEntryForm.addEventListener('submit', function(event) {
            ...
            if (eventDateInput.value && !isSunday(eventDateInput.value)) {
                alert('Please correct the date. It must be a Sunday.');
